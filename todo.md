...
## V4.1.1: PostgreSQL for SDDMini-KH Deployment
- [x] Update docker-compose.yml to use PostgreSQL 16 instead of MySQL 8.0
- [x] Update ENV_REFERENCE.md with PostgreSQL connection string defaults and Docker vars
- [x] Update CLAUDE_RUNNER_PROMPT.md architecture diagram and deployment steps for PostgreSQL
- [x] Create drizzle/schema-pg.ts with PostgreSQL-compatible schema (pgTable, pgEnum, serial)
- [x] Update server/db.ts with dual-dialect auto-detection (postgresql:// vs mysql://)
- [x] Update drizzle.config.ts to auto-detect dialect from DATABASE_URL
- [x] All 53 tests pass — push to GitHub with updated tag
